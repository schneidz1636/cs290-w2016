...
$tempURL = 'https://www.googleapis.com/books/v1/volumes?q=isbn:'.htmlspecialchars($isbn_temp).'&key=AIzaSyBfO0dyPVYsGyHWzxHUVJGiLVa3fcQVTmI';
$json = file_get_contents($tempURL);
$bookstring = json_decode($json);

if($bookstring->totalItems == "0"){
	echo "<script>location.replace(\"bad_isbn.php?isbn_temp=$isbn_temp\");</script>";
}
